...
def sanitize_concept_name(concept):
    """Sanitize concept name for Windows compatibility"""
    # Replace invalid characters with underscores
    invalid_chars = r'[<>:"/\\|?*]'
    sanitized = re.sub(invalid_chars, '_', concept)
    # Remove trailing dots and spaces
    sanitized = sanitized.rstrip('. ')
    # Make paths shorter to avoid Windows path length issues
    if len(sanitized) > 50:
        hash_suffix = hashlib.md5(concept.encode()).hexdigest()[:8]
        sanitized = f"{sanitized[:30]}_{hash_suffix}"
    return sanitized
...
def train_binary_model(concept, mapping, features, save_path):
    ...
    model_save_path = f"{save_path}/{sanitized_concept}"
    ...
    sanitized_concept_filename = sanitize_concept_name(concept)
    with open(f"{model_save_path}/{sanitized_concept_filename}_results.txt", 'w') as f:
        f.write(f"{train_score},{val_score}")

    # save model
    pickle.dump(model, open(f"{model_save_path}/{sanitized_concept_filename}.p", 'wb'))
...
if __name__ == "__main__":
    ...
    if args.modality == "xray":
        ...
        # For skin lesions, we'll use the concept annotations directly
        # Load the concept annotations from the bottlenecks directory
        with open(f'./data/bottlenecks/concept_priors/skin_class2concept2answer.json', 'r') as f:
            concept_annotations = json.load(f)
        
        # Create a simplified structure for skin lesion training
        features = {}
        metadata = {}
        annotations = {}
        
        # Load features from available skin lesion datasets
        skin_datasets = ['HAM10000', 'ISIC-age', 'ISIC-color', 'ISIC-hospital', 'ISIC-sex', 'ISIC-site', 'Melanoma', 'PAD-UFES-20', 'UWaterloo']
        
        for dataset in skin_datasets:
            try:
                dataset_features = torch.load(f'./data/features/whylesionclip/{dataset}_label.pt', weights_only=False)
                features.update(dataset_features)
                print(f"Loaded features for {dataset}: {len(dataset_features)} classes")
            except FileNotFoundError:
                print(f"Features not found for {dataset}, skipping...")
                continue
        
        # Create a simplified mapping for skin lesions
        # We'll use the concept annotations to create positive/negative mappings
        for concept in concept_annotations:
            annotations[concept] = {"positive": [], "negative": []}
            # For now, we'll create a simple mapping based on the concept answers
            # This is a simplified approach - in practice, you'd need proper image-level annotations
