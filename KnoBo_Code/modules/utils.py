...
def load_classifier_list(args):
    ...
    if args.bottleneck == "random":
        ...
        for c in concepts:
            sanitized_c = sanitize_concept_name(c)
            model_full_path = f"{model_path}/{sanitized_c}/{sanitized_c}.p"
            # check existence of the model
            if not os.path.exists(model_full_path):
                print(f"Model for {c} does not exist")
                continue
            # Try to find the results file with any name ending in _results.txt
            results_files = [f for f in os.listdir(f"{model_path}/{sanitized_c}") if f.endswith("_results.txt")]
            if not results_files:
                print(f"Results file for {c} not found")
                continue
                
            results_file = results_files[0]  # Use the first results file found
            
            curr_model = pickle.load(open(model_full_path, "rb"))

            with open(f"{model_path}/{sanitized_c}/{results_file}", "r") as f:
